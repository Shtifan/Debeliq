...
module.exports = {
    ...
    async execute({ inter }) {
        ...
        if (!track && !number) inter.reply({ content: `You have to use one of the options to jump to a song ${inter.member} ❌`, ephemeral: true })
        ...
        if (number) {
            ...
            if (!trackname) return inter.reply({ content: `This track dose not seem to exist ${inter.member} ❌`, ephemeral: true })
            queue.skipTo(index)
